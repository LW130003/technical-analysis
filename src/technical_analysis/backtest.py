...
def run_backtest(df: pd.DataFrame, profit_target: float = 0.03, stop_loss: float = 0.03, max_hours: int = 48) -> pd.DataFrame:
    ...
    for start_idx in trade_indices:
        ...
        # We look ahead by max_hours, but filter to ensure we don't bleed into another ticker
        forward_window = df.loc[start_idx + 1 : start_idx + max_hours]
        forward_window = forward_window[forward_window['ticker'] == ticker]
        
        if len(forward_window) < max_hours:
            # Check if this is truly the end of the stock's data history, or just a truncated chunk
            actual_ticker_rows_left = len(df.loc[start_idx + 1:]) - len(df.loc[start_idx + 1:][df['ticker'] != ticker])
            
            # If your forward window is short because the data literally cut off:
            ...
            # Max profit and loss potential achieved DURING each hour
